...
.. list-table:: Reset behavior based on LCS ``ROT`` or ``DEPLOYED``
   ...
   * - ``RESET_VIA_SECDOM``
     - Uses the CTRL-AP mailbox to send a local domain reset request to the Secure Domain Firmware (SDFW).
       ...
       This resets the whole system, but keeps the application and radio core halted.
       They can be started individually with ``nrfutil device go --core Network`` and ``nrfutil device go --core Application``.

       This is particularly useful for debugging individual domains from their reset handlers onward.
   * - ``RESET_DEFAULT``
     - Selects ``RESET_HARD``.
