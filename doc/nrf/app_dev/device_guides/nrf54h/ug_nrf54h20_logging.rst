...
* Logging with System Trace Macrocell (STM)
...
The STM multiplexes trace protocol data, formatted according to the MIPI System Trace Protocol (STP) v2, and sends it to a sink, like a ETR or TPIU.
...
#. The STM multiplexes the data stream with streams from other domains and potentially other sources such as ETM.
#. The data flows through a funnel and replication network to a sink, either TPIU or ETR.

Logging using STM leverages the frontend API of the Zephyr logging subsystem to select the stimulus ports and writes log messages directly to memory-mapped registers.
...
The Trace Port Interface Unit (TPIU) is a hardware feature that allows outputting the trace data onto a 5 pin Trace connector that can be captured by an external tool such as the Segger J-Trace.
The captured date must be processed by suitable PC tools that can offer instruction tracing in the case of ETM or logging when using ``nrfutil trace`.
...
The configuration of the different components of the Test and Debug Domain (TDDCONF) happens via the tddconf device tree binding. You can check `snippets/nordic-log-stm/boards/nrf54h20_cpuapp.overlay` for a sample.
Basic idea is that one enables the sources for a sink as a bitmask. The different sources are defined in `include/zephyr/dt-bindings/misc/nordic-tddconf.h`.

Sources:
   * NRF_TDDCONF_SOURCE_STMMAINCORE - Enable STM for the main core, either app or radio depending on the build target
   * NRF_TDDCONF_SOURCE_ETMMAINCORE - Enable ETM (instruction trace) for the main core, either app or radio depending on the build target
   * NRF_TDDCONF_SOURCE_STMHWEVENTS - Enable HW events
   * NRF_TDDCONF_SOURCE_STMPPR - Enable STM for PPR, only valid for app core as it is the owner for PPR
   * NRF_TDDCONF_SOURCE_STMFLPR - Enable STM for PPR, only valid for app core as it is the owner for PPR

Sinks:
   * etrsources - All the sources enabled to be routed towards ETR
   * tpiusources - All the sources enabled to be routed towards TPIU

.. note::
   The data flows through a network of funnels and replicators, the sources and sinks are in some ways additive so you might see data on both sinks.

Standalone logging via ETR + UART
=================================

The application core, acting as the ETR-designated proxy, reads the ETR buffer and decodes the raw encoded log data.
The logging data in a human-readable format is then transferred over UART.
...
Dictionary-based logging via ETR + UART
=======================================
...
Dictionary logging via TPIU
===========================

Follow the guide for "Dictionary-based logging via ETR + UART" but in `snippets/nordic-log-stm/boards/nrf54h20_cpuapp.overlay` and `snippets/nordic-log-stm/boards/nrf54h20_cpurad.overlay` replace the <etrsources> with <tpiusources>.
This will route everything to the TPIU sink.

   Use the ``nrfutil trace stm`` command to start capturing logs from the device, specifying the database configuration for each domain ID, along with the serial number of the connected J-Link::

      nrfutil trace stm --database-config <domain_id>:build/<app_name>/zephyr/log_dictionary.json --input-jtrace-serial-number <JLink serial number> --stdout ascii
