...
int suit_processor_get_manifest_metadata_invalid_fake_func(uint8_t *envelope_str,
				size_t envelope_len, bool authenticate, struct zcbor_string *manifest_component_id,
...
int suit_processor_get_manifest_metadata_correct_fake_func(uint8_t *envelope_str,
				size_t envelope_len, bool authenticate, struct zcbor_string *manifest_component_id,
				struct zcbor_string *digest, enum suit_cose_alg *alg, unsigned int *seq_num)
{
	zassert_equal(TEST_ENVELOPE_ADDRESS, envelope_str, "Invalid envelope_str");
							      ...
							      suit_component_type_t *type)
{
	...
	*type = SUIT_COMPONENT_TYPE_CACHE_POOL + 30; /* Arbitrary value added to get type out of bounds */
...
{
	zassert_equal(valid_component_handle, handle, "Unexpected component handle value");
	zassert_not_equal(impl_data, NULL, "API must provide valid address to get imeplementation-specific data");
...
suit_plat_err_t suit_digest_sink_get_correct_fake_func(struct stream_sink *sink, psa_algorithm_t algorithm, const uint8_t *expected_digest)
{
...
suit_plat_err_t suit_digest_sink_get_invalid_fake_func(struct stream_sink *sink, psa_algorithm_t algorithm, const uint8_t *expected_digest)
{
	zassert_not_null(sink, "The API must provide a valid pointer");
	zassert_not_null(expected_digest, "The API must provide a valid pointer");
...
suit_plat_err_t suit_generic_address_streamer_stream_correct_fake_func(const uint8_t *payload, size_t payload_size,
...
suit_plat_err_t suit_generic_address_streamer_stream_invalid_fake_func(const uint8_t *payload, size_t payload_size,
						     struct stream_sink *sink)
{
	zassert_not_null(payload, "The API must provide a valid pointer");
	zassert_not_equal(payload_size, 0, "The API must provide a valid payload size");
...
ZTEST(suit_plat_check_image_match_tests, test_instld_mfst_OK)
{
	...
	suit_plat_retrieve_manifest_fake.custom_fake = suit_plat_retrieve_manifest_correct_fake_func;
	suit_processor_get_manifest_metadata_fake.custom_fake = suit_processor_get_manifest_metadata_correct_fake_func;

	int ret = suit_plat_check_image_match(valid_component_handle, suit_cose_sha256, &valid_digest);
	...
	zassert_equal(suit_plat_retrieve_manifest_fake.call_count, 1, "Incorrect number of suit_plat_retrieve_manifest() calls");
	zassert_equal(suit_processor_get_manifest_metadata_fake.call_count, 1, "Incorrect number of suit_processor_get_manifest_metadata() calls");
}

ZTEST(suit_plat_check_image_match_tests, test_cand_mfst_OK)
{
	suit_plat_component_id_get_fake.custom_fake = suit_plat_component_id_get_correct_fake_func;
	suit_plat_decode_component_type_fake.custom_fake = suit_plat_decode_component_type_cand_mfst_correct_fake_func;
	suit_plat_retrieve_manifest_fake.custom_fake = suit_plat_retrieve_manifest_correct_fake_func;
	suit_processor_get_manifest_metadata_fake.custom_fake = suit_processor_get_manifest_metadata_correct_fake_func;
...
ZTEST(suit_plat_check_image_match_tests, test_instld_mfst_get_manifest_metadata_fail)
{
	...
	zassert_equal(SUIT_ERR_DECODING, ret, "Check should have failed - suit_processor_get_manifest_metadata returned error");
...
ZTEST(suit_plat_check_image_match_tests, test_instld_mfst_retrieve_manifest_fail)
{
	...
	zassert_equal(SUIT_ERR_UNSUPPORTED_COMPONENT_ID, ret, "Check should have failed - suit_plat_retrieve_manifest returned error");

	/* Check expected call counts for fake functions */
	zassert_equal(suit_plat_component_id_get_fake.call_count, 1, "Incorrect number of suit_plat_component_id_get() calls");
	zassert_equal(suit_plat_decode_component_type_fake.call_count, 1, "Incorrect number of suit_plat_decode_component_type() calls");
	zassert_equal(suit_plat_retrieve_manifest_fake.call_count, 1, "Incorrect number of suit_plat_retrieve_manifest() calls");
	zassert_equal(suit_processor_get_manifest_metadata_fake.call_count, 0, "Incorrect number of suit_processor_get_manifest_metadata() calls");
}

ZTEST(suit_plat_check_image_match_tests, test_decode_component_type_invalid_type_fail)
{
	suit_plat_component_id_get_fake.custom_fake = suit_plat_component_id_get_correct_fake_func;
	suit_plat_decode_component_type_fake.custom_fake = suit_plat_decode_component_type_invalid_type_ret_fake_func;
	suit_plat_retrieve_manifest_fake.custom_fake = suit_plat_retrieve_manifest_invalid_fake_func;
	suit_processor_get_manifest_metadata_fake.custom_fake = suit_processor_get_manifest_metadata_invalid_fake_func;

	int ret = suit_plat_check_image_match(valid_component_handle, suit_cose_sha256, &valid_digest);

	zassert_equal(SUIT_ERR_UNSUPPORTED_COMPONENT_ID, ret, "Check should have failed - suit_plat_decode_component_type returned invalid type");

	/* Check expected call counts for fake functions */
	zassert_equal(suit_plat_component_id_get_fake.call_count, 1, "Incorrect number of suit_plat_component_id_get() calls");
	zassert_equal(suit_plat_decode_component_type_fake.call_count, 1, "Incorrect number of suit_plat_decode_component_type() calls");
	zassert_equal(suit_plat_retrieve_manifest_fake.call_count, 0, "Incorrect number of suit_plat_retrieve_manifest() calls");
	zassert_equal(suit_processor_get_manifest_metadata_fake.call_count, 0, "Incorrect number of suit_processor_get_manifest_metadata() calls");
}
...
{
	suit_plat_component_id_get_fake.custom_fake = suit_plat_component_id_get_correct_fake_func;
	suit_plat_decode_component_type_fake.custom_fake = suit_plat_decode_component_type_unsupported_correct_fake_func;
	suit_plat_retrieve_manifest_fake.custom_fake = suit_plat_retrieve_manifest_invalid_fake_func;
	suit_processor_get_manifest_metadata_fake.custom_fake = suit_processor_get_manifest_metadata_invalid_fake_func;
...
{
	suit_plat_component_id_get_fake.custom_fake = suit_plat_component_id_get_correct_fake_func;
	suit_plat_decode_component_type_fake.custom_fake = suit_plat_decode_component_type_invalid_fake_func;
	suit_plat_retrieve_manifest_fake.custom_fake = suit_plat_retrieve_manifest_invalid_fake_func;
	suit_processor_get_manifest_metadata_fake.custom_fake = suit_processor_get_manifest_metadata_invalid_fake_func;

	int ret = suit_plat_check_image_match(valid_component_handle, suit_cose_sha256, &valid_digest);

	zassert_equal(SUIT_ERR_UNSUPPORTED_COMPONENT_ID, ret, "Check should have failed - suit_plat_decode_component_type returned error");
...
ZTEST(suit_plat_check_image_match_tests, test_component_id_get_fail)
{
	...
	zassert_equal(SUIT_ERR_UNSUPPORTED_COMPONENT_ID, ret, "Check should have failed - suit_plat_component_id_get returned error");

	/* Check expected call counts for fake functions */
	zassert_equal(suit_plat_component_id_get_fake.call_count, 1, "Incorrect number of suit_plat_component_id_get() calls");
	zassert_equal(suit_plat_decode_component_type_fake.call_count, 0, "Incorrect number of suit_plat_decode_component_type() calls");
	zassert_equal(suit_plat_retrieve_manifest_fake.call_count, 0, "Incorrect number of suit_plat_retrieve_manifest() calls");
	zassert_equal(suit_processor_get_manifest_metadata_fake.call_count, 0, "Incorrect number of suit_processor_get_manifest_metadata() calls");
}
...
{
	suit_plat_component_id_get_fake.custom_fake = suit_plat_component_id_get_correct_fake_func;
	suit_plat_decode_component_type_fake.custom_fake = suit_plat_decode_component_type_special_correct_fake_func;
	suit_plat_decode_component_number_fake.custom_fake = suit_plat_decode_component_number_correct_fake_func;
	...
	zassert_equal(SUIT_ERR_UNSUPPORTED_COMPONENT_ID, ret, "Check should have failed for HW other than NRF54H20");
#endif /* CONFIG_SOC_NRF54H20 */

	/* Check expected call counts for fake functions */
	zassert_equal(suit_plat_component_id_get_fake.call_count, 1, "Incorrect number of suit_plat_component_id_get() calls");
	zassert_equal(suit_plat_decode_component_type_fake.call_count, 1, "Incorrect number of suit_plat_decode_component_type() calls");
	zassert_equal(suit_plat_decode_component_number_fake.call_count, 1, "Incorrect number of suit_plat_decode_component_number() calls");
}
...
ZTEST(suit_plat_check_image_match_tests, test_soc_spec_invalid_component_number_ret)
{
	suit_plat_component_id_get_fake.custom_fake = suit_plat_component_id_get_correct_fake_func;
	suit_plat_decode_component_type_fake.custom_fake = suit_plat_decode_component_type_special_correct_fake_func;
	suit_plat_decode_component_number_fake.custom_fake = suit_plat_decode_component_number_invalid_ret_number_fake_func;

	int ret = suit_plat_check_image_match(valid_component_handle, suit_cose_sha256, &valid_digest);

	zassert_equal(SUIT_ERR_UNSUPPORTED_COMPONENT_ID, ret, "Check should have failed - invalid component number");
...
{
	suit_plat_component_id_get_fake.custom_fake = suit_plat_component_id_get_correct_fake_func;
	suit_plat_decode_component_type_fake.custom_fake = suit_plat_decode_component_type_mem_correct_fake_func;
	...
	suit_generic_address_streamer_stream_fake.custom_fake = suit_generic_address_streamer_stream_correct_fake_func;
	suit_digest_sink_digest_match_fake.custom_fake = suit_digest_sink_digest_match_correct_fake_func;
...
ZTEST(suit_plat_check_image_match_tests, test_mem_sink_digest_match_fail)
{
	...
	suit_digest_sink_get_fake.custom_fake = suit_digest_sink_get_correct_fake_func;
	suit_generic_address_streamer_stream_fake.custom_fake = suit_generic_address_streamer_stream_correct_fake_func;
	suit_digest_sink_digest_match_fake.custom_fake = suit_digest_sink_digest_match_invalid_fake_func;
...
ZTEST(suit_plat_check_image_match_tests, test_mem_generic_address_streamer_stream_fail)
{
	...
	suit_memptr_storage_ptr_get_fake.custom_fake = suit_memptr_storage_ptr_get_correct_fake_func;
	suit_digest_sink_get_fake.custom_fake = suit_digest_sink_get_correct_fake_func;
	suit_generic_address_streamer_stream_fake.custom_fake = suit_generic_address_streamer_stream_invalid_fake_func;
	suit_digest_sink_digest_match_fake.custom_fake = suit_digest_sink_digest_match_invalid_fake_func;
